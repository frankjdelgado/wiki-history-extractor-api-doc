...
Para llevar a cabo el cumplimiento de los objetivos planteados en el Capítulo 2, es necesario definir un esquema o metodología de trabajo que permita la elaboración de un conjunto de
...
La figura \ref{fig:diagram_general} muestra la arquitectura general del servicio web, y como la misma interactuá con las aplicaciones clientes que lo consumen.
...
\item Y, por último, un componente de almacenamiento de artículos wiki y sus revisiones. Para ello se hace uso de MongoDB y esta implementado como un cluster de base de datos distribuidos
...
La ruta \texttt{articles} tiene como finalidad devolver la información pertinente a los artículos que han sido extraídos a través del API. Para mostrar la información, se realiza una consulta a la base de datos, específicamente la colección \texttt{articles}. Debido a que esta operación no implica cálculo de métricas, esta consulta es realizada directamente, sin ser asignada a los nodos trabajadores de celery.
...
Un ejemplo de la respuesta obtenida por esta ruta puede ser apreciada en la figura \ref{fig:response_articles}:
...
En la tabla \ref{tab:articles} se puede apreciar la historia de la ruta.

\input{tables/articles}

Por su parte, la ruta \texttt{revisions} tiene como finalidad devolver la información pertinente de las revisiones que han sido extraídas. En comparación con la información de la ruta \texttt{articles}, la información en \texttt{revisions} es mucho mas detallada, y abarca desde el nombre del usuario que hizo la revisión, hasta la fecha en que se realizó y el contenido completo de la revisión. Como sucede con los artículos, la información de las revisiones es presentada gracias a una consulta a la colección \texttt{revisions} de la base de datos. De igual manera, la consulta de las revisiones no necesita calcular métricas, y por ello la consulta es realizada directamente.

Para esta ruta, adicionalmente a los parámetros de búsqueda, está el parámetro \texttt{sort} que permite, por fecha de extracción, el
ordenamiento ascendente o descendiente de las revisiones, y la elección de cuáles
...
En la tabla \ref{tab:revisions} se puede apreciar con mas detalle los argumentos que pueden ser usados para filtrado, paginación y ordenamiento.

\input{tables/revisions}

Las rutas anteriormente mencionadas son rutas de acceso de consulta, es decir, sólo muestran datos recolectados y que se encuentran almacenados en la base de datos. Por otro lado, existen mas rutas de acceso al API para la consulta de diversas métricas predefinidas, tales como: promedio, conteo de revisiones y moda.

Para el cálculo de estas métricas, se toman como parámetros múltiples atributos, tales como: el título del artículo o la fecha de extracción. Dichos atributos son usados principalmente para reducir o filtrar las revisiones que se van a seleccionar para el cálculo de las métricas. 

Antes de que los parámetros recibidos en la consulta sean utilizados, los mismos pasan por un proceso de aceptación, en donde sólo son aceptados aquellos que estén ya incluidos en una lista, la cual es denominada \textit{Lista Blanca}, mientras que el resto son ignorados.

Después de esta etapa, se genera una tarea de Celery con el tipo de consulta a realizar y los parámetros recibidos que fueron aceptados a través de la lista blanca. Esta tarea pasa a ser procesada en segundo plano.

Dentro de esta tarea, los atributos recibidos como parámetros son usados en sus diferentes funciones, dependiendo de la consulta que fue realizada. 

Para el uso de los filtros en una consulta, algunos de los atributos pasan por un proceso en el cual son convertidos a condicionales para que las revisiones puedan ser seleccionadas correctamente. Entre dichos atributos, se tienen las fechas y el tamaño de la revisión. Para las fechas, se considera una fecha única o un intervalo de fechas en base a los parámetros que fueron recibidos, y se añaden los condicionales correspondientes. Para el tamaño sucede un proceso similar, donde se considera un segundo argumento opcional para crear las condiciones. 

Con las condiciones ya ajustadas(en caso de que fuera necesario), se anexan a un objeto, que es pasado como parámetro a la función que se conectará con la base de datos, y hará la consulta. 

La consulta de conteo de revisiones, tiene por finalidad seleccionar todas las revisiones que cumplan con las condiciones de los parámetros recibidos. Todos los argumentos de esta consulta son utilizados como filtros para las revisiones a seleccionar. En la tabla \ref{tab:count} se puede apreciar con detalle, por medio de su historia, los filtros aceptados por dicha consulta.

\input{tables/count}

La consulta del promedio de las revisiones, calcula el promedio de revisiones por unidad de tiempo(en este caso, días) que existe en un intervalo determinado por el usuario a través de los parámetros. Adicionalmente al intervalo de fechas para para seleccionar el promedio, se pueden aplicar los diversos filtros disponibles, para afinar aún mas la selección de las revisiones, por ejemplo, para poder ver el promedio de revisiones de un usuario en un fin de semana determinado. En la tabla \ref{tab:average} se presentan con mayor con detalle, por medio de su historia, los filtros aceptados, así como los argumentos obligatorios de la consulta.

\input{tables/average}

La consulta de la moda de las revisiones, calcula el valor que se repite con mayor frecuencia de un atributo seleccionado por el usuario. Dicha consulta se realiza sobre un conjunto de revisiones que puede ser filtrado, tal como las consultas anteriores, lo que permite una mayor especificación de las revisiones a considerar para el cálculo de la moda, por ejemplo, calcular cuál es el mayor contribuyente a un artículo en un año determinado. Para mas detalles de los filtros y argumentos de la consulta, se presenta la tabla \ref{tab:mode}.
...
La arquitectura elegida para esta solución contiene la cantidad de elementos mínimos para implementar un cluster de base de datos en MongoDB, y cuenta con: dos sets de réplicas de shards, un set de réplicas para el servidor de configuración, y por último, un servidor de consultar.
...
Esta separación de los datos puede ser apreciada en la figura \ref{fig:sharded_database} que se presenta a continuación:
